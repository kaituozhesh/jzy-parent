...
public class CartServiceImpl implements CartService {
    ...
    public List<Cart> addGoodsToCartList(List<Cart> cartList, Long itemId, Integer num) {
        ...
        // 可能在用户查看商品的时候   状态还是1     可能在用户点击添加购物车的时候  商品的状态变成了不合法
        // 需要  注意时间差的问题
        ...
        if (cart == null) {
            ...
            // 5.1 如果不存在该明细,创建新的购物车明细对象,并添加到该购物车的明细列表中
            if (orderItem == null) {
                orderItem = createOrderItem(item, num);
                cart.getOrderItemList().add(orderItem);
            } else {// 5.2 如果存在该明细对象,在原有的数量上添加数量,并且更新金额
                // 更改数量
    ...
    /**
     *  根据skuID 在购物车明细列表中查询购物车明细对象
     * @param orderItemsList
     * @param itemId
     * @return
     */
    public TbOrderItem searchOrderItemByItemId(List<TbOrderItem> orderItemsList, Long itemId){
    ...
    public List<Cart> findCartListFromRedis(String username) {
        System.out.println(username + ": 从redis中提取购物车");
    ...
    public void saveCartListToRedis(String username, List<Cart> cartList) {
        System.out.println(username + ": 向redis中存入购物车");
